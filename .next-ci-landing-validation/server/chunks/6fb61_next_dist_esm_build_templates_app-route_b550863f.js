...
WHERE id = $1
LIMIT 1
    `.trim(),[t])).rows[0]??null}async function v(e,t){let a=await e.query(`
SELECT cancel_requested
FROM strategy_lab_runs
WHERE id = $1
LIMIT 1
    `.trim(),[t]);return!!a.rows[0]?.cancel_requested}async function S(e,t){await e.query(`
...
SET status = 'dead_letter',
    ...
    updated_at = now()
WHERE id = $1
      `.trim(),[s.jobId]),{status:"failed",runId:s.runId,jobId:s.jobId};if(i.cancel_requested)return await S(e.client,{runId:s.runId,jobId:s.jobId,reason:"cancel_requested_before_start"}),{status:"cancelled",runId:s.runId,jobId:s.jobId};await e.client.query(`
UPDATE strategy_lab_jobs
SET status = 'running',
    attempt = attempt + 1,
    updated_at = now()
...
WHERE id = $1
    `.trim(),[s.runId]),await x(e.client,{runId:s.runId,jobId:s.jobId,eventType:"run_started",payload:{status:"running",workerId:e.workerId,attempt:s.attempt+1,maxAttempts:s.maxAttempts}});let o=new AbortController,d=setInterval(()=>{(async()=>{await t.heartbeat(e.client,{jobId:s.jobId,workerId:e.workerId,leaseMs:r}),await v(e.client,s.runId)&&!o.signal.aborted&&o.abort()})()},n);try{let t=(0,R.normalizeCreateRunRequest)(i.request_payload),a=await (0,R.executeRunInput)(s.runId,t,{signal:o.signal});return await e.client.query(`
UPDATE strategy_lab_runs
...
SET status = 'succeeded',
    ...
    finished_at = now()
WHERE id = $1
      `.trim(),[s.jobId]),await x(e.client,{runId:s.runId,jobId:s.jobId,eventType:"run_succeeded",payload:{status:"succeeded",totalTrades:a.result.trades.length,totalReturn:a.result.metrics.totalReturn}}),{status:"succeeded",runId:s.runId,jobId:s.jobId}}catch(r){var u;let t=(0,R.getExecutionErrorMeta)(r);if("RUN_ABORTED"===t.code||await v(e.client,s.runId))return await S(e.client,{runId:s.runId,jobId:s.jobId,reason:"RUN_ABORTED"===t.code?"abort_signal":"cancel_requested_during_execution"}),{status:"cancelled",runId:s.runId,jobId:s.jobId};let a=s.attempt+1;if(u=t.code,A.has(u)&&a<s.maxAttempts){let r,n=(r=f(e.retryBaseDelayMs??1e3,100,6e4),Math.min(f(e.retryMaxDelayMs??1e4,r,3e5),Math.trunc(r*Math.pow(2,Math.max(0,a-1)))));return await e.client.query(`
UPDATE strategy_lab_runs
SET status = 'queued',
    error_code = $2,
    error_message = $3,

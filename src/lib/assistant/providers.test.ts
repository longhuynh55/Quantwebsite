...
describe("assistant providers response_format behavior", () => {
  ...
  it("does not attach response_format when schema mode env is unset", async () => {
    ...
    const result = await generateWithProviderFallback(
      ...
      { requestId: "req-1", responseFormat: RESPONSE_FORMAT }
    );
  ...
  it("attaches response_format when schema mode env is enabled", async () => {
    ...
    const result = await generateWithProviderFallback(
      [{ role: "user", content: "test" }],
      { requestId: "req-2", responseFormat: RESPONSE_FORMAT }
    );
  ...
  it("falls back after 422 and caches unsupported schema capability", async () => {
    ...
    const first = await generateWithProviderFallback(
      [{ role: "user", content: "first" }],
      { requestId: "req-3", responseFormat: RESPONSE_FORMAT }
    );
    ...
    const second = await generateWithProviderFallback(
      [{ role: "user", content: "second" }],
      { requestId: "req-4", responseFormat: RESPONSE_FORMAT }
    );

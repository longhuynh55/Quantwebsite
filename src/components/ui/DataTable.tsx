    ...
    return (
      <div className="w-full flex flex-col gap-2">
        {Array.from({ length: 10 }).map((_, i) => (
          <div key={i} className="h-12 w-full bg-gray-100 dark:bg-slate-800 animate-pulse rounded-lg" />
        ))}
      </div>
    );
  }
    ...
    <div className="rounded-xl border border-gray-200 dark:border-slate-800 overflow-hidden bg-white dark:bg-slate-900 shadow-sm">
      <div className="overflow-x-auto">
        <table className="w-full border-collapse text-sm">
          <thead className="sticky top-0 z-20 bg-gray-50 dark:bg-slate-800 border-b border-gray-200 dark:border-slate-700">
            <tr>
              {columns.map((column, idx) => (
                <th
                  key={idx}
                  className={cn(
                    "px-4 py-3 text-xs font-bold uppercase tracking-wider text-gray-500 dark:text-slate-400 transition-colors",
          ...
          {rowVirtualizer.getVirtualItems().map((virtualRow) => {
            ...
            return (
              <div
                ...
                className={cn(
                  "absolute top-0 left-0 w-full flex border-b border-gray-100 dark:border-slate-800/50 hover:bg-blue-50/30 dark:hover:bg-blue-500/5 transition-colors cursor-pointer group",
